export async function onRequest(context) {
  ...
  // 1. ASSET PASS-THROUGH (Handles photos, fonts, and favicon.png)
  ...
  let images = [];
  if (isHome) {
    // HOMEPAGE FILTER: Only load these specific folders. 
    // IMPORTANT: These names must exactly match your R2 folder names (including spaces or underscores).
    const homeFoldersToLoad = ['BTS', 'Old Work']; 
    
    // Fetch all specified folders simultaneously for speed
    const folderPromises = homeFoldersToLoad.map(folderName => 
      env.PHOTOS_BUCKET.list({ prefix: `${folderName}/` })
    );
    const folderResults = await Promise.all(folderPromises);
    
    // Combine the results into a single flat array
    ...
    // FOLDER PAGES: Load specific folder contents
